...
{
    public enum DP12MST_TBDataType { MainLinkSingleThread, MainLinkMultiThread, AuxiliaryLinkSingleThread, Unknown }

    //[ConfigComponent(1)]
    public class DP12MSTTraceBufferDataMgr : ITBObject
    {
        #region Members

        private enum DP12MST_TBDataMgrMode { SingleThreaded, MultiThreaded, Unknown }
        private enum LinkID { Main, Auxiliary, Unknown };

        private class DP12MST_TBParameters
        {
        ...
        private class DP12MST_AuxParameters
        {
        ...
        public class DP12MST_TBUploadDescriptor
        {
            ...
            public DP12MST_TBUploadDescriptor(long startState, long endState, long triggerState, long triggerOffset, long numOfStates, byte wrapped,
        ...
        public class DP12MST_TBSearchDescriptor
        {
            ...
            public DP12MST_TBSearchDescriptor(DP12MST_TRACE_BUFFER_MODE listingType, string cName, uint cValue, int vChannelID, long stateIndex, bool searchForward)
            {
                TBMode = listingType;
                ColumnName = cName;
                ColumnValue = cValue;
        ...
        private class DP12MST_TaskStateObject
        {
            ...
            //public DP12MST_TaskStateObject(string path, long trigStateOffset, long startStateIndex, long[] fileStartIndices, long blockSize, long memoryDepth, long numOfCapturedStates, FT60x_IF interfaceRef, HWDataUploadStatusEvent dataUploadStatusEvent, int index, CancellationToken ct)
            //{
            //    FolderPath = path;
        ...
        private class DP12MST_ColumnMetaData
        {
        ...
        /// </summary>
        private class DP12MST_MultiThread_MetaDataArgs
        {
       ...
       // private CancellationToken token = CancellationToken.None;
        ...
        private const string MainLink_COLUMNS_DEFS_XML_FILENAME = "DP12MST_TBDataFormat.xml";
        private const string AuxiliaryLink_COLUMNS_DEFS_XML_FILENAME = "DP12MST_AuxTBDataFormat.xml";
        ...
        private const string CONFIG_XML_FILENAME = "Config_DP12MSTTraceBufferDataMgrForm.xml";
        ...
        private DP12MST_TRACE_BUFFER_MODE m_TBMode = DP12MST_TRACE_BUFFER_MODE.MainLink;        // Main, Auxiliary, Both or Unknown
        private DP12MST_TBDataMgrMode m_TBDataMode = DP12MST_TBDataMgrMode.MultiThreaded;      // singleThreaded, MultiThreaded, unknown

        // the idea is that only one task will update this variable.
        private volatile string m_triggerBinaryFileName = string.Empty;

        //private List<DP12MST_MultiThread_MetaDataArgs> m_multiThreadFileMetaData_VC1 = new List<DP12MST_MultiThread_MetaDataArgs>();
        //private List<DP12MST_MultiThread_MetaDataArgs> m_multiThreadFileMetaData_VC2 = new List<DP12MST_MultiThread_MetaDataArgs>();
        //private List<DP12MST_MultiThread_MetaDataArgs> m_multiThreadFileMetaData_VC3 = new List<DP12MST_MultiThread_MetaDataArgs>();
        //private List<DP12MST_MultiThread_MetaDataArgs> m_multiThreadFileMetaData_VC4 = new List<DP12MST_MultiThread_MetaDataArgs>();

        private List<DP12MST_VCFileGenerator_Threads.DP12MST_MultiThread_MetaDataArgs> m_multiThreadFileMetaData = new List<DP12MST_VCFileGenerator_Threads.DP12MST_MultiThread_MetaDataArgs>();

        private List<DP12MST_ColumnMetaData> m_columnMetaData_MainLink = new List<DP12MST_ColumnMetaData>();
        private List<DP12MST_ColumnMetaData> m_columnMetaData_AuxiliaryLink = new List<DP12MST_ColumnMetaData>();
        ...
        //
        public event DP12MSTEvent DisplayPort12MSTEvent;
        ...
        private DP12MST_TBParameters m_TBParameters = null;
        ...
        private DP12MST_AuxParameters m_AUXParameters = null;
        ...
        //private DP12MST_VCFileGenerator m_VCFileGenerator = null;
        ...
        /// Default Constructor
        /// </summary>
        public DP12MSTTraceBufferDataMgr(string path)
        {
            //m_FTD2xxIF = FT60x_IF.GetInstance();
            m_DP12MSTMessageGenerator = DP12MSTMessageGenerator.GetInstance();
            //m_VCFileGenerator = DP12MST_VCFileGenerator.GetInstance();
            ...
            //getColumnMetaData(DP12MST_TRACE_BUFFER_MODE.MainLink);
            //getColumnMetaData(DP12MST_TRACE_BUFFER_MODE.AuxiliaryLink);
        }
        ...
        //    {
        //        long stateIndex = -1;
        //        if (((DP12MST_TBSearchDescriptor)e.Argument).SearchForward)
        //        {
        //            stateIndex = searchListing_SingleThread_Forward(((DP12MST_TBSearchDescriptor)e.Argument).TBMode,
        //                                                            ((DP12MST_TBSearchDescriptor)e.Argument).ColumnName,
        //                                                            ((DP12MST_TBSearchDescriptor)e.Argument).ColumnValue,
        //                                                            ((DP12MST_TBSearchDescriptor)e.Argument).StateIndex,
        //                                                            SEARCH_REPORTING_INTERVAL_SIZE);
        //        }
        //        else  // search backwards
        //        {
        //            stateIndex = searchListing_SingleThread_Backward(((DP12MST_TBSearchDescriptor)e.Argument).TBMode,
        //                                                            ((DP12MST_TBSearchDescriptor)e.Argument).ColumnName,
        //                                                            ((DP12MST_TBSearchDescriptor)e.Argument).ColumnValue,
        //                                                            ((DP12MST_TBSearchDescriptor)e.Argument).StateIndex,
        //                                                            SEARCH_REPORTING_INTERVAL_SIZE);
        ...
        //                            new DP12MSTEventArgs(m_DP12MSTMessageGenerator.DP12MSTMessageCreate_SearchListingProgessReport(DP12MST_TRACE_BUFFER_MODE.MainLink, SEARCH_REPORTING_INTERVAL_SIZE)));
        //    }
        //}
        ...
        //    DP12MST_TRACE_BUFFER_MODE TBMode = DP12MST_TRACE_BUFFER_MODE.MainLink;
        //    if (m_MainLinkSingleThreadSearch == false)
        //        TBMode = DP12MST_TRACE_BUFFER_MODE.AuxiliaryLink;
        ...
        //        if (((DP12MST_TBSearchDescriptor)e.Argument).SearchForward)
        //        {
        //            stateIndex = searchListing_MultiThread_Forward(((DP12MST_TBSearchDescriptor)e.Argument).TBMode,
        //                                                            ((DP12MST_TBSearchDescriptor)e.Argument).ColumnName,
        //                                                            ((DP12MST_TBSearchDescriptor)e.Argument).ColumnValue,
        //                                                            ((DP12MST_TBSearchDescriptor)e.Argument).VChannelID,
        //                                                            ((DP12MST_TBSearchDescriptor)e.Argument).StateIndex,
        //                                                            SEARCH_REPORTING_INTERVAL_SIZE);
        //        }
        //        else  // search backwards
        //        {
        //            stateIndex = searchListing_MultiThread_Backward(((DP12MST_TBSearchDescriptor)e.Argument).TBMode,
        //                                                            ((DP12MST_TBSearchDescriptor)e.Argument).ColumnName,
        //                                                            ((DP12MST_TBSearchDescriptor)e.Argument).ColumnValue,
        //                                                            ((DP12MST_TBSearchDescriptor)e.Argument).VChannelID,
        //                                                            ((DP12MST_TBSearchDescriptor)e.Argument).StateIndex,
        ...
        //        // raise an event  -- informing the top level object that the upload is done!
        //        if (DisplayPort12MSTEvent != null)
        //            DisplayPort12MSTEvent(this, new DP12MSTEventArgs((m_DP12MSTMessageGenerator.DP12MSTMessageCreate_SearchListingResponse(DP12MST_TRACE_BUFFER_MODE.MainLink, 0, true, (long)(e.Result)))));
        //    }
        //    else
        //    {
        //        // raise an event  -- informing the top level object that the upload is done!
        //        if (DisplayPort12MSTEvent != null)
        //            DisplayPort12MSTEvent(this, new DP12MSTEventArgs((m_DP12MSTMessageGenerator.DP12MSTMessageCreate_SearchListingResponse(DP12MST_TRACE_BUFFER_MODE.MainLink, 0, false, (long)(e.Result)))));
        //    }
        ...
        private void DoWork_AuxUpload(object sender, DoWorkEventArgs e)
        {
            ...
            //else
            //{
            //    DP12MST_TBUploadDescriptor parameters = (DP12MST_TBUploadDescriptor)e.Argument;
        ...
        /////                                                    "TIME_COUNT:50", "TRIG_LINE:31" };
        ///// <returns></returns>
        //private bool GetTBStatusVariables(List<long> fldValues, DP12MST_TBParameters parameters)
        //{
        //    bool status = true;
        //    long pValue = 0x00;
        ...
        /////                                                    "TIME_COUNT:50", "TRIG_LINE:31" };
        ///// <returns></returns>
        //private bool GetAUXTBStatusVariables(List<long> fldValues, DP12MST_AuxParameters parameters)
        //{
        //    bool status = true;
        //    long pValue = 0x00;
        ...
        //    m_TBParameters = new DP12MST_TBParameters();
        ...
        //        status = m_fldExtractor.GetloopFields(m_TBStatusFieldWidths, loopData, fldValues, 2, loopData.Length - 3);  //startIndex, endIndex);
        //    }
        ...
        //    m_fldExtractor = new FieldExtractor();
        //    m_AUXParameters = new DP12MST_AuxParameters();
        ...
        //private void initSearchBGWorker_SingleThreaded(DP12MST_TBSearchDescriptor threadParameters)
        //{
        ...
        ///// </summary>
        ///// <param name="threadParameters"></param>
        //private void initSearchBGWorker_MultiThreaded(DP12MST_TBSearchDescriptor threadParameters)
        //{
        ...
        ///// 
        //private void initAuxLinkBGWorker(DP12MST_TBUploadDescriptor threadParameters)
        //{
        //    //if (m_auxWorker != null)
        //    //{
        //    //    m_auxWorker.Dispose();
        ...
        ///// </summary>
        ///// <param name="threadParameters"></param>
        //private void uploadData_BGWorkerMode(DP12MST_TBUploadDescriptor threadParameters, bool isMainLinkData = true)
        //{
        ...
        ///// <returns></returns>
        //private bool waitForCascadingTasksToComplete(Task<DP12MST_TaskStateObject>[] tasks,
        //                                                Task<DP12MST_TaskStateObject>[][] continuationTasks)
        //{
        ...
        //private bool waitForCascadingTasksToComplete(Task<DP12MST_TaskStateObject>[] tasks,
        //                                                Task<DP12MST_TaskStateObject>[] continuationTasks,
        //                                                Task<DP12MST_TaskStateObject>[] continuationTasks2,
        //                                                Task<DP12MST_TaskStateObject>[] continuationTasks3,
        //                                                int numOfLevels = 4,
        ...
        //                                                Task<DP12MST_TaskStateObject>[] continuationTasks2,
        //                                                Task<DP12MST_TaskStateObject>[] continuationTasks3,
        //                                                Task<DP12MST_TaskStateObject>[] continuationTasks4,
        //                                                Task<DP12MST_TaskStateObject>[] continuationTasks5,
        //                                                Task<DP12MST_TaskStateObject>[] continuationTasks6,
        //                                                Task<DP12MST_TaskStateObject>[] continuationTasks7,
        //                                                int numOfLevels = 8,
        //                                                int numOfTaskPerGroup = 4)
        //{
        //    bool waitToExit = true;
        ...
        ///// <param name="numOfLevels"></param>
        ///// <param name="threadParameters"></param>
        //private async void uploadDataFromHardware(int numOfColumns, int numOfLevels, DP12MST_TBUploadDescriptor threadParameters)
        //{
        //    ////
        ...
        //    //        }, new DP12MST_TaskStateObject(folderPath, m_FTD2xxIF.TrigOffset, m_FTD2xxIF.StartStateIndex,
        ...
        //    //                ((DP12MST_TaskStateObject)antecedent.Result).Token.ThrowIfCancellationRequested();

        //    //                // get the data...
        //    //                createTraceBufferDataFileII(((DP12MST_TaskStateObject)antecedent.Result).FolderPath + tls.Value.Index.ToString() + ".bin",
        //    //                                            ((DP12MST_TaskStateObject)antecedent.Result).TrigStateIndex,
        //    //                                            ((DP12MST_TaskStateObject)antecedent.Result).StartStateIndex,
        //    //                                            ((DP12MST_TaskStateObject)antecedent.Result).FileStartIndices[tls.Value.Index],
        //    //                                            ((DP12MST_TaskStateObject)antecedent.Result).BlockSize,
        //    //                                            ((DP12MST_TaskStateObject)antecedent.Result).MemoryDepth,
        //    //                                            ((DP12MST_TaskStateObject)antecedent.Result).NumOfCapturedStates,
        //    //                                            ((DP12MST_TaskStateObject)antecedent.Result).MyFTD2xxIF,
        //    //                                            mutex,
        //    //                                            ((DP12MST_TaskStateObject)antecedent.Result).Token,
        //    //                                            ((DP12MST_TaskStateObject)antecedent.Result).UploadStatusEvent);

        //    //                return new DP12MST_TaskStateObject(tls.Value.FolderPath, tls.Value.TrigStateIndex, tls.Value.StartStateIndex,
        ...
        //    //        removeDataFiles(DP12MST_TRACE_BUFFER_MODE.MainLink);
        ...
        //                status = getFieldParameters(DP12MST_TRACE_BUFFER_MODE.MainLink, TIMESTAMP_FIELD_NAME, ref fldWidth, ref fldOffset);
        //                if (status)
        ...
        //                status = getFieldParameters(DP12MST_TRACE_BUFFER_MODE.MainLink, VCTAG_FIELD_NAME, ref fldWidth, ref fldOffset);
        //                if (status)
        //                    status = LoopOperations.GetFieldLocation(TB_STATE_BYTE_LEN, fldOffset, ref byteID, ref bitID);
        //                if (status)
        ...
        ///// <returns></returns>
        //private int getNumberOfColumns(DP12MST_TRACE_BUFFER_MODE mode, long memoryDepth)
        //{
        //    int numOfColumns = 0;
        //    if (mode == DP12MST_TRACE_BUFFER_MODE.MainLink)
        //    {
        //        if (memoryDepth <= TB_MAX_THREAD_SIZE_SMALL)
        //        {
        ...
        //        }
        //    }
        //    else if (mode == DP12MST_TRACE_BUFFER_MODE.AuxiliaryLink)
        //    {
        ...
        private bool getDataFromHardware()
        {
            ...
            //DP12MST_TBUploadDescriptor threadParameters = new DP12MST_TBUploadDescriptor(m_FTD2xxIF.StartStateIndex,
        ...
        {
            bool status = false; // true;
            //DP12MST_TBUploadDescriptor threadParameters = new DP12MST_TBUploadDescriptor(m_FTD2xxIF.StartStateIndex_Aux,
            ...
            //m_TBDataMode = DP12MST_TBDataMgrMode.SingleThreaded;
        ...
        ///// </summary>
        ///// <returns></returns>
        //public List<DP12MST_VCFileGenerator_Threads.DP12MST_MultiThread_MetaDataArgs> getMultiThreadedFileMetaData()
        //{
        ...
        /// </summary>
        /// <param name="DisplayPortEvent"></param>
        private void getRequestedData(DP12MSTMessage DisplayPortEvent)
        {
            bool status = false;

            if (m_TBDataMode == DP12MST_TBDataMgrMode.SingleThreaded)
            {
                // if the TB file exists...
                if (File.Exists(m_traceFilePath))
                {
                    if (m_currentStateNumber != ((DP12MSTMessage_StateDataRequest)DisplayPortEvent).StateNumber)
                    {
                        // clear the previous state data 
                        Array.Clear(m_currentStateData, 0, m_currentStateData.Length);

                        // get the requested state data
                        status = getState(((DP12MSTMessage_StateDataRequest)DisplayPortEvent).VChannelID, ((DP12MSTMessage_StateDataRequest)DisplayPortEvent).StateNumber, (int)m_dataOffset_mainLink);
                        if (!status)
                        {
            ...
            else if (m_TBDataMode == DP12MST_TBDataMgrMode.MultiThreaded)
            {
                status = getState(((DP12MSTMessage_StateDataRequest)DisplayPortEvent).VChannelID, ((DP12MSTMessage_StateDataRequest)DisplayPortEvent).StateNumber);
                if (!status)
                {
                    m_currentStateNumber = long.MinValue;
                    Array.Clear(m_currentStateData, 0, m_currentStateData.Length);
            ...
            if (DisplayPort12MSTEvent != null)
            {
                // Raise the response event containing the requested data
                DisplayPort12MSTEvent(this,
                                    new DP12MSTEventArgs(m_DP12MSTMessageGenerator.DP12MSTMessageCreate_StateDataResponse(
                                                            ((DP12MSTMessage_StateDataRequest)DisplayPortEvent).ID,
                                                            ((DP12MSTMessage_StateDataRequest)DisplayPortEvent).VChannelID,
                                                            ((DP12MSTMessage_StateDataRequest)DisplayPortEvent).StateNumber,
                                                            m_currentStateData)));
            }
        }

        private void getRequestedDataChunk(DP12MSTMessage DisplayPortEvent)
        {
            bool status = false;
            DP12MSTMessage_StateDataChunkRequest msg = DisplayPortEvent as DP12MSTMessage_StateDataChunkRequest;
            ...
            {
                status = getStateChunk((DP12MSTMessage_StateDataChunkRequest)DisplayPortEvent);
                if (!status)
                {
                    m_currentStateNumber = long.MinValue;
                    Array.Clear(m_currentStateData, 0, m_currentStateData.Length);
        ...
        /// <param name="DisplayPortEvent"></param>
        /// <returns></returns>
        public bool getRequestedStateData(DP12MSTMessage DisplayPortEvent)
        {
            bool status = false;
            DP12MSTMessage_GetStateData msg = DisplayPortEvent as DP12MSTMessage_GetStateData;
            byte[] dataBytes = new byte[TB_STATE_BYTE_LEN];

            if (DisplayPortEvent is DP12MSTMessage_GetStateData)
            {
                if (m_TBDataMode == DP12MST_TBDataMgrMode.SingleThreaded)
                {
                    // if the TB file exists...
        ...
        /// <param name="DisplayPortEvent"></param>
        /// <returns></returns>
        public bool getRequestedAuxStateData(DP12MSTMessage DisplayPortEvent)
        {
            bool status = false;
            DP12MSTMessage_AuxGetStateData msg = DisplayPortEvent as DP12MSTMessage_AuxGetStateData;
            byte[] dataBytes = new byte[TB_STATE_BYTE_LEN];

            if (DisplayPortEvent is DP12MSTMessage_AuxGetStateData)
            {
                if (m_TBDataMode == DP12MST_TBDataMgrMode.SingleThreaded)
                {
                    // if the TB file exists...
        ...
        ///// <returns></returns>
        //private List<DP12MST_MultiThread_MetaDataArgs> getMultiThreadFileDataRef(int vChannelID)
        //{
        //    List<DP12MST_MultiThread_MetaDataArgs> multiThreadFileMetaData = null;
        ...
        private bool getStateChunk(DP12MSTMessage_StateDataChunkRequest msg)
        {
            bool status = false;
        ...
        //    if (File.Exists(fileName))
        //    {
        //        List<DP12MST_MultiThread_MetaDataArgs> multiThreadFileMetaData = getMultiThreadFileDataRef(vChannelID);
        ...
        /// <param name="stateIndex"></param>
        /// <returns></returns>
        private int getFileIndex_inclusive(DP12MST_TRACE_BUFFER_MODE TBMode, long stateIndex, int vChannelID)
        {
            bool status = true;
            int fileIndex = -1;
            string path = string.Empty;
        ...
        /// </summary>
        /// <returns></returns>
        private List<DP12MST_VCFileGenerator_Threads.DP12MST_MultiThread_MetaDataArgs> initVChannelMetaData(int numOfDataFiles)
        {
            bool status = true;

            List<DP12MST_VCFileGenerator_Threads.DP12MST_MultiThread_MetaDataArgs> vChannelMetaData = new List<DP12MST_VCFileGenerator_Threads.DP12MST_MultiThread_MetaDataArgs>();
            string path = string.Empty;
            int delimiterCount = 0;
            ...
            for (int i = 0; i < numOfDataFiles / 4; i++)                    // 1 Millon states has 16 blocks, each with 4 channels ==> (16 * 4) = 64 files need to be processed
            {
                for (int channelID = 1; channelID <= 4; channelID++)        // each column should have four channels
                {
                    ...
                    if (File.Exists(path))
                    {
                        ...
                        DP12MST_VCFileGenerator_Threads.DP12MST_MultiThread_MetaDataArgs metaDataArgs = new DP12MST_VCFileGenerator_Threads.DP12MST_MultiThread_MetaDataArgs();
        ...
        //private List<DP12MST_MultiThread_MetaDataArgs> initVChannelMetaData(int vChannelID, List<DP12MST_MultiThread_MetaDataArgs> vChannelMetaData)
        //{
        //    bool status = true;
        //    string path = string.Empty;
        //    int delimiterCount = 0;
        ...
        //        {
        //            // create an object to hold the meta data for the current file.
        //            DP12MST_MultiThread_MetaDataArgs metaDataArgs = new DP12MST_MultiThread_MetaDataArgs();
        ...
        /// Assumption:  this method extracts the meta data for the data files for the specified channel, 
        ///              NOT the meta data for all data files...
        private bool getMultiThreadedFileMetaData(DP12MSTMessage_GetTBBinFileMetaData msg)
        {
            bool status = true;
            string path = string.Empty;
            int delimiterCount = 0;
        ...
        /// <param name="msg"></param>
        /// <returns></returns>
        private bool getSingleThreadedFileMetaData(string path, DP12MSTMessage_GetTBBinFileMetaData msg)
        {
            bool status = true;
            int delimiterCount = 0;
            int parameterValue = -1;
            ...
            if (msg.TBMode == DP12MST_TRACE_BUFFER_MODE.AuxiliaryLink)
            ...
            if (File.Exists(filePath))
            {
                ...
                using (TextReader sr = File.OpenText(path))
                {
                    ...
                    while (((s = sr.ReadLine()) != null) && (delimiterCount < 1) && (status == true))
                    {
                        ...
                        if (msg.TBMode == DP12MST_TRACE_BUFFER_MODE.MainLink)
        ...
        /// </summary>
        /// <returns></returns>
        private DP12MST_TBDataMgrMode getMainLinkTBDataFormat()
        {
           DP12MST_TBDataMgrMode mode = DP12MST_TBDataMgrMode.Unknown;

            if (File.Exists(m_traceFilePath))
            {
                mode = DP12MST_TBDataMgrMode.SingleThreaded;
            }
            else
            {
                ...
                if (File.Exists(path))
                    mode = DP12MST_TBDataMgrMode.MultiThreaded;
            }
        ...
        /// <param name="path"></param>
        /// <returns></returns>
        private bool getTBMetaData(DP12MSTMessage DisplayPortEvent)
        {
            bool status = true;

            if (((DP12MSTMessage_GetTBBinFileMetaData)DisplayPortEvent).TBMode == DP12MST_TRACE_BUFFER_MODE.MainLink)
            {
                m_TBDataMode = getMainLinkTBDataFormat();
                if (m_TBDataMode == DP12MST_TBDataMgrMode.SingleThreaded)
                {
                    status = getSingleThreadedFileMetaData(m_traceFilePath, ((DP12MSTMessage_GetTBBinFileMetaData)DisplayPortEvent));
                }
                else if (m_TBDataMode == DP12MST_TBDataMgrMode.MultiThreaded)
                {
                    status = getMultiThreadedFileMetaData((DP12MSTMessage_GetTBBinFileMetaData)DisplayPortEvent);
                }
            }
            else if (((DP12MSTMessage_GetTBBinFileMetaData)DisplayPortEvent).TBMode == DP12MST_TRACE_BUFFER_MODE.AuxiliaryLink)
            {
                getSingleThreadedFileMetaData(m_auxTraceFilePath, ((DP12MSTMessage_GetTBBinFileMetaData)DisplayPortEvent));
            }
        ...
        /// <returns></returns>
        private bool getFieldParameters(DP12MST_TRACE_BUFFER_MODE TBMode, string fieldName, ref int width, ref int offset)
        {
            bool status = false;

            List<DP12MST_ColumnMetaData> metaDataList = m_columnMetaData_MainLink;
            if (TBMode == DP12MST_TRACE_BUFFER_MODE.AuxiliaryLink)
                metaDataList = m_columnMetaData_AuxiliaryLink;


            foreach (DP12MST_ColumnMetaData metaData in metaDataList)
            {
        ...
        /// <returns></returns>
        private bool searchListing_SingleThead(DP12MSTMessage_SearchListingRequest searchMsg, ref long locatedStateIndex)
        {
            bool found = false;
            ...
            //DP12MST_TBSearchDescriptor threadParameters = new DP12MST_TBSearchDescriptor(searchMsg.TBMode, searchMsg.ColumnName, searchMsg.m_ColumnHexValue, 
        ...
        /// <param name="locatedStateIndex"></param>
        /// <returns></returns>
        private bool searchListing_MultiThead(DP12MSTMessage_SearchListingRequest searchMsg, ref long locatedStateIndex)
        {
            bool found = false;
            //bool searchForward = true;
        ...
        /// <param name="locatedStateIndex"></param>
        /// <returns></returns>
        private long searchListing_MultiThread_Forward(DP12MST_TRACE_BUFFER_MODE TBMode, string cName, uint cValue, int vChannelID, long stateIndex, int interval = 1024)
        {
            bool status = false;
            bool found = false;
            int fldWidth = 0x00;
            ...
            if (fileIndex >= 0)
            {
                ...
                status = getFieldParameters(DP12MST_TRACE_BUFFER_MODE.MainLink, cName, ref fldWidth, ref fldOffset);
                if (status)
                    status = LoopOperations.GetFieldLocation(TB_STATE_BYTE_LEN, fldOffset, ref byteID, ref bitID);
            }
        ...
        /// <param name="locatedStateIndex"></param>
        /// <returns></returns>
        private long searchListing_MultiThread_Backward(DP12MST_TRACE_BUFFER_MODE TBMode, string cName, uint cValue, int vChannelID, long stateIndex, int interval = 1024)
        {
            bool status = false;
            bool found = false;
            int fldWidth = 0x00;
        ...
        private long searchListing_SingleThread_Forward(DP12MST_TRACE_BUFFER_MODE TBMode, string cName, uint cValue, long stateIndex, int interval = 1024)
        {
            bool found = false;
            int fldWidth = 0x00;
            int fldOffset = 0x00;
            ...
            string filePath = m_traceFilePath;
            DP12MST_TRACE_BUFFER_MODE traceBufferMode = DP12MST_TRACE_BUFFER_MODE.MainLink;
            //if (searchMsg.TBMode == TRACE_BUFFER_MODE.AuxiliaryLink)
            if (TBMode == DP12MST_TRACE_BUFFER_MODE.AuxiliaryLink)
            {
                filePath = m_auxTraceFilePath;
                traceBufferMode = DP12MST_TRACE_BUFFER_MODE.AuxiliaryLink;
            }
        ...
        private long searchListing_SingleThread_Backward(DP12MST_TRACE_BUFFER_MODE TBMode, string cName, uint cValue, long stateIndex, int interval = 1024)
        {
            bool found = false;
            int fldWidth = 0x00;
            int fldOffset = 0x00;
        ...
        /// <param name="DisplayPortEvent"></param>
        /// <returns></returns>
        private bool searchTraceBufferData(DP12MSTMessage DisplayPortEvent)
        {
            bool found = false;
            long stateIndex = -1;

            if (DisplayPortEvent is DP12MSTMessage_SearchListingRequest)
            {
                DP12MSTMessage_SearchListingRequest searchMsg = DisplayPortEvent as DP12MSTMessage_SearchListingRequest;
                if (searchMsg.TBMode == DP12MST_TRACE_BUFFER_MODE.MainLink)
                {
                    if (m_TBDataMode == DP12MST_TBDataMgrMode.SingleThreaded)
                    {
                ...
                else if (searchMsg.TBMode == DP12MST_TRACE_BUFFER_MODE.AuxiliaryLink)
                {
                    m_MainLinkSingleThreadSearch = false;
                    found = searchListing_SingleThead(searchMsg, ref stateIndex);
                }
        ...
        /// <param name="DisplayPortEvent"></param>
        /// <returns></returns>
        private bool removeDataFiles(DP12MST_TRACE_BUFFER_MODE TBModeID)
        {
            bool status = true;
            int maxRetries = 10;
            for (int attempt = 0; attempt < 5; attempt++)
            {
                try
                {
                    if (TBModeID == DP12MST_TRACE_BUFFER_MODE.MainLink)
                    {
                        //DirectoryInfo di = new DirectoryInfo(m_FS4500_FOLDER_PATH + "\\" + m_FS4500_FOLDER_NAME);
                        DirectoryInfo di = new DirectoryInfo(m_instanceFolderPath);
                    ...
                    else if (TBModeID == DP12MST_TRACE_BUFFER_MODE.AuxiliaryLink)
                    {
                        if (File.Exists(m_auxTraceFilePath))
                            File.Delete(m_auxTraceFilePath);
                    }
                    else if (TBModeID == DP12MST_TRACE_BUFFER_MODE.Both)
                    {
        ...
        /// <returns></returns>
        private List<string> getDataFileNames(DP12MST_TBDataType linkID)
        {
            List<string> fileNames = new List<string>();
            ...
            foreach (string name in names)
            {
                ...
                {
                    if (linkID == DP12MST_TBDataType.AuxiliaryLinkSingleThread)
                    {
                    ...
                    {
                        if (linkID == DP12MST_TBDataType.MainLinkSingleThread)
                        {
        ...
        /// <returns></returns>
        private bool getColumnMetaInfo(string path, ref List<DP12MST_ColumnMetaData> columnMetaDataList)
        {
            bool status = true;
            ...
            using (XmlReader reader = XmlReader.Create(path))
            {
                ...
                while (reader.Read())
                {
                    ...
                    if (reader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        {
                            DP12MST_ColumnMetaData cInfo = new DP12MST_ColumnMetaData();
        ...
        /// </summary>
        /// <param name="TBMode"></param>
        private void getColumnMetaData(DP12MST_TRACE_BUFFER_MODE TBMode)
        {
            string xmlfilePath = "";
            if (TBMode == DP12MST_TRACE_BUFFER_MODE.MainLink)
            {
        ...
        private bool createConfigXMLFile(string filePath)
        {
            ...
            try
            {
                ...
                List<string> mainLinkFileNames_Single = getDataFileNames(DP12MST_TBDataType.MainLinkSingleThread);
                List<string> mainLinkFileNames_Multi = getDataFileNames(DP12MST_TBDataType.MainLinkMultiThread);
                List<string> auxLinkFileNames_Single = getDataFileNames(DP12MST_TBDataType.AuxiliaryLinkSingleThread);
        ...
        {
            if (DisplayPort12MSTEvent != null)
                DisplayPort12MSTEvent(this, new DP12MSTEventArgs((DP12MSTMessage)(m_DP12MSTMessageGenerator.DP12MSTMessageCreate_Register((ITBObject)this))));
        }
        ...
        /// </summary>
        /// <param name="DisplayPortEvent"></param>
        public void ProcessDPMessage(DP12MSTMessage DisplayPortEvent)
        {
            if (DisplayPortEvent is DP12MSTMessage_PMRunning)
            {
            ...
            else if (DisplayPortEvent is DP12MSTMessage_PMStopping_Request)
            {
                //if (tokenSource != null)
            ...
            else if (DisplayPortEvent is DP12MSTMessage_PMStopping_Ack)
            {
                //TBD
            }
            else if (DisplayPortEvent is DP12MSTMessage_PMReady)
            {
                // do nothing... the event was generated from this object!
            }
            else if (DisplayPortEvent is DP12MSTMessage_DataAvailable)
            {
                //m_ExitRunMode = ((DP12MSTMessage_DataAvailable)DisplayPortEvent).ExitRunMode;
                //uploadTraceBufferData(DisplayPortEvent);
            }
            else if (DisplayPortEvent is DP12MSTMessage_DataUploaded)
            {
            ...
            else if (DisplayPortEvent is DP12MSTMessage_StateDataRequest)
            {
                // retain the state for subsequent requests to the same state... saves on overhead.
                getRequestedData(DisplayPortEvent);
            }

            else if (DisplayPortEvent is DP12MSTMessage_StateDataChunkRequest)
            {
                getRequestedDataChunk(DisplayPortEvent);
            }
            else if (DisplayPortEvent is DP12MSTMessage_GetStateData)
            {
            ...
            else if (DisplayPortEvent is DP12MSTMessage_AuxDataAvailable)
            {
                //m_ExitRunMode = ((DP12MSTMessage_AuxDataAvailable)DisplayPortEvent).ExitRunMode;
                //uploadAuxTraceBufferData(DisplayPortEvent);
            }
            else if (DisplayPortEvent is DP12MSTMessage_AuxStateDataRequest)
            {
                //// retain the state for subsequent requests to the same state... saves on overhead.
                //getAuxRequestedData(DisplayPortEvent);
            }
            else if (DisplayPortEvent is DP12MSTMessage_AuxGetStateData)
            {
                //getRequestedAuxStateData(DisplayPortEvent);
            }
            else if (DisplayPortEvent is DP12MSTMessage_MemoryDepthChanged)
            {
                //// retain the state for subsequent requests to the same state... saves on overhead.
            ...
            else if (DisplayPortEvent is DP12MSTMessage_TimeSlotSelectionChanged)
            {
            ...
            //}
            else if (DisplayPortEvent is DP12MSTMessage_TBDataModeChange)
            {
                //m_TBMode = ((DP12MSTMessage_TBDataModeChange)DisplayPortEvent).TBModeID;
            }
            else if (DisplayPortEvent is DP12MSTMessage_GetTBBinFileMetaData)
            {
                getTBMetaData(DisplayPortEvent);
            }
            else if (DisplayPortEvent is DP12MSTMessage_SearchListingRequest)
            {
                //searchTraceBufferData(DisplayPortEvent);
            }
            else if (DisplayPortEvent is DP12MSTMessage_SearchListingCancelRequest)
            {
                //cancelSearch();
            }
            else if (DisplayPortEvent is DP12MSTMessage_Initialize)
            {
                //if (((DP12MSTMessage_Initialize)DisplayPortEvent).RemoveDataFiles)
                //{
                //    removeDataFiles(DP12MST_TRACE_BUFFER_MODE.MainLink);
                //    removeDataFiles(DP12MST_TRACE_BUFFER_MODE.AuxiliaryLink);
                //}
            }
            else if (DisplayPortEvent is DP12MSTMessage_GetNumberOfStates)
            {
