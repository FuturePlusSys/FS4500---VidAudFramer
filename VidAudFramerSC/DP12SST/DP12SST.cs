...
{
    public enum DP12SST_TRACE_BUFFER_MODE { MainLink, AuxiliaryLink, Both, Unknown }

    public enum DP12SST_CROSS_LINK_MODE { AuxiliaryLink, MainLink, Unknown }

    public class DP12SST : IProbeMgrGen2
    {
        #region Members
        ...
        private static DP12SST_MessagePump m_eventPump;
        private static DP12SST_TraceBufferDataMgr m_TBDataMgr;
        ...
        /// Default Constructor
        /// </summary>
        public DP12SST(string path)
        {
            m_eventPump = new DP12SST_MessagePump();

            m_TBDataMgr = new DP12SST_TraceBufferDataMgr(path);
            m_TBDataMgr.DisplayPort12SSTEvent += new DP12SSTEvent(processDPEvent);
        ...
        /// <param name="e"></param>
        private void processDPEvent(object sender, DP12SSTEventArgs e)
        {
            if (e.Message is DP12SSTMessage_TBObjectRegister)
            {
                DP12SSTMessage_TBObjectRegister regEvent = e.Message as DP12SSTMessage_TBObjectRegister;
                m_eventPump.AddTBObjectRef(regEvent.TBObj);
            }
            else if (e.Message is DP12SSTMessage_StateDataResponse)
            {
        ...
        /// <returns></returns>
        private bool extractStateDataFromResponseMessage(DP12SSTMessage DisplayPortEvent)
        {
            bool status = true;
            ...
            if (m_stateData == null)
            {
                m_stateData = new byte[((DP12SSTMessage_StateDataResponse)DisplayPortEvent).DataBytes.Length];
            }
            else if ((m_stateData.Length == ((DP12SSTMessage_StateDataResponse)DisplayPortEvent).DataBytes.Length))
            {
                Array.ConstrainedCopy(((DP12SSTMessage_StateDataResponse)DisplayPortEvent).DataBytes, 0, m_stateData, 0, ((DP12SSTMessage_StateDataResponse)DisplayPortEvent).DataBytes.Length);
            }
            else
            {
                m_stateData = new byte[((DP12SSTMessage_StateDataResponse)DisplayPortEvent).DataBytes.Length];
        ...
        public bool Configure(int deviceNum, string serialNumberStr, bool inDemoMode)
        {
            ...
            //analyzerType = DP12SST_LA_TYPE.agilent;
        ...
        public int GetTriggerChannelID()
        {
            ...
            //DP12SSTMessage msg = m_DP12SSTMessageGenerator.DP12SSTMessageCreate_GetTBBinFileMetaData();
            //((DP12SSTMessage_GetTBBinFileMetaData)msg).TBMode = DP12SST_TRACE_BUFFER_MODE.MainLink;  // main or auxiliary link
            ////((DP12SSTMessage_GetTBBinFileMetaData)msg).VChannelID = -1;
            ...
            //VChannelID = 0; // ((DP12SSTMessage_GetTBBinFileMetaData)msg).VChannelID;
        ...
        public long GetTriggerStateIndex(int virtualChannelID = 0)
        {
            ...
            return ((DP12SSTMessage_GetTBBinFileMetaData)msg).TriggerOffset;
        }
        ...
        {
            //m_TBDataMgr.LoadModuleConfig("Config_DP12SSTTraceBufferDataMgrForm.xml");  // should we make an additional IProbeMgr I/F method to load the module?


            DP12SSTMessage msg = m_DP12SSTMessageGenerator.DP12SSTMessageCreate_GetTBBinFileMetaData();
            ((DP12SSTMessage_GetTBBinFileMetaData)msg).TBMode = DP12SST_TRACE_BUFFER_MODE.MainLink;  // main or auxiliary link
            //((DP12SSTMessage_GetTBBinFileMetaData)msg).VChannelID = virtualChannelID;


            m_eventPump.ForwardEvent(msg);      // the answer will be in the next statement before this one ends...
                                                // that is because .NET events are synchronous and blocking...

            return (((DP12SSTMessage_GetTBBinFileMetaData)msg).NumberOfStates);
        }
        ...
        public byte[] GetStateData(int virtualChannelID, int index)
        {
            ...
            DP12SSTMessage_GetStateData msg = new DP12SSTMessage_GetStateData(0, index);
        ...
        {
            DP12SSTMessage_StateDataChunkRequest msg = m_DP12SSTMessageGenerator.DP12SSTMessageCreate_StateDataChunkRequest(0, startIndex, chunkSize);
            m_eventPump.ForwardEvent(msg);

            return msg.DataChunk;  // stubbed code...
        }
    ...
    public delegate void DP12SSTEvent(object sender, DP12SSTEventArgs e);

    /// <summary>
    /// Derived Event Args used to contain DP Message objects 
    /// </summary>
    public class DP12SSTEventArgs : EventArgs
    {
        public DP12SSTMessage Message;

        public DP12SSTEventArgs(DP12SSTMessage message)
        {
