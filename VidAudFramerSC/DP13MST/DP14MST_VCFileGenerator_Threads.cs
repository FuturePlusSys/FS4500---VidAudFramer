...
{
    public class DP14MSTVCFileGenerator_Threads
    {
        #region Members
        ...
        private class DP14MSTTaskStateObject
        {
            ...
            public DP14MSTTaskStateObject(string path, VCFileGenerationStatusEvent fileGenStatusEvent, float percentage, int index, CancellationToken ct)
            {
                FolderPath = path;
            ...
            public DP14MSTTaskStateObject()
            {
            }
        }


        private class DP14MSTTrigLocationTaskStateObject
        {
            public VCFileGenerationStatusEvent FileGenerationStatusEvent { get; set; }
            ...
            public DP14MSTTrigLocationTaskStateObject(long trigTimeStamp,  int trigChannelID, VCFileGenerationStatusEvent fileGenStatusEvent, float percentage, CancellationToken ct)
            {
        ...
        private class DP14MSTGetMetaDataTaskStateObject
        {
            ...
            public DP14MSTGetMetaDataTaskStateObject(int numOfColumns, int numOfLevels, int numOfChannels, int triggerIndex, int triggerVChannelID, VCFileGenerationStatusEvent fileGenStatusEvent, float percentage, CancellationToken ct)
            {
        ...
        private class DP14MSTGetTrigStateIndicesTaskStateObject
        {
            ...
            public DP14MSTGetTrigStateIndicesTaskStateObject(UploadMetaData[] metaDataRef, long trigTimeStamp, VCFileGenerationStatusEvent fileGenStatusEvent, float percentage, CancellationToken ct)
            {
        ...
        private class DP14MSTPrependMetaTaskStateObject
        {
            ...
            public DP14MSTPrependMetaTaskStateObject(int channelID, UploadMetaData[] metaDataRef, VCFileGenerationStatusEvent fileGenStatusEvent, float percentage, CancellationToken ct)
            {
                ChannelID = channelID;
                MetaData = metaDataRef;
                FileGenerationStatusEvent = fileGenStatusEvent;
        ...
        private class UploadMetaData
        {
            ...
            public void Copy(DP14MSTMultiThread_MetaDataArgs destMetaData)
            {
        ...
        /// </summary>
        public class DP14MSTMultiThread_MetaDataArgs
        {
        ...
        public DP14MSTVCFileGenerator_Threads()
        {
        }
        #endregion // Ctor
        ...
        private void clearDataFiles()
        {
            ...
            string[] filePaths = Directory.GetFiles(m_instanceFolderPath);
        ...
        private void clearVCDataFiles()
        {
            ...
            if (m_debugDataFiles)  // for testing
                clearFolder(m_instanceFolderPath + "\\debug\\");
                ...
                if (fileName.StartsWith("TraceData")) 
                {
                    if (!fileName.Contains("_VC_"))
                    {
                        if (m_debugDataFiles)  // for testing and trouble shooting
                            File.Move(path, m_instanceFolderPath + "\\debug\\" + fileName);
        ...
        private void createVirtualChannelFiles(string binaryFilePath, int blockIndex, CancellationToken token, VCFileGenerationStatusEvent fileGenerationStatusEvent, float progressPercentage)
        {
            using (FileStream fs = new FileStream(binaryFilePath, FileMode.Open))
            {
                ...
                string vc1_FilePath = m_instanceFolderPath + "\\TraceData_" + blockIndex.ToString() + "_VC_1.bin";
                string vc2_FilePath = m_instanceFolderPath + "\\TraceData_" + blockIndex.ToString() + "_VC_2.bin";
                string vc3_FilePath = m_instanceFolderPath + "\\TraceData_" + blockIndex.ToString() + "_VC_3.bin";
                string vc4_FilePath = m_instanceFolderPath + "\\TraceData_" + blockIndex.ToString() + "_VC_4.bin";
        ...
        private bool prependMetaData(int channelID, UploadMetaData[] metaDataList, VCFileGenerationStatusEvent fileGenerationStatusEvent, float progressPercentage)
        {
            ...
            for (int i = channelID; i < metaDataList.Length; i += 4)  // loop for the number of columns
            {
                ...
                consolidatedDataFileName = m_instanceFolderPath + "\\" + BLOCK_VC_FILENAME_BASE + (i / 4).ToString() + "_VC_" + (channelID + 1).ToString() + ".bin";
                string tempFile = m_instanceFolderPath + "\\" + BLOCK_VC_FILENAME_BASE + channelID.ToString() + "_temp.bin";
        ...
        private UploadMetaData[]  getConsolidatedFilesMetaData(int numOfColumns, int numOfLevels, int numOfChannels, int triggerVChannelID, int triggerIndex, VCFileGenerationStatusEvent fileGenerationStatusEvent, float progressPercentage)
        {
            ...
            for (int channelID = 1; channelID <= numOfChannels; channelID++)
            {
                ...
                for (int blockID = 0; blockID < numOfBlocks; blockID++)
                {
                    ...
                    consolidatedDataFileName = m_instanceFolderPath + "\\" + BLOCK_VC_FILENAME_BASE + blockID.ToString() + "_VC_" + channelID.ToString() + ".bin";
        ...
        private UploadMetaData[] setMetaDataTriggerStateIndices(UploadMetaData[] metaDataList, long triggerTimeStamp, VCFileGenerationStatusEvent fileGenerationStatusEvent, float progressPercentage)
        {
            ...
            for (int channelID = 1; channelID <= 4; channelID++)        // each column should have four channels
            {
                ...
                for (int i = 0; i < metaDataList.Length / 4; i++)       // 1 Millon states has 16 blocks, each with 4 channels ==> (16 * 4) = 64 files need to be processed
                {
                    ...
                    consolidatedDataFileName = m_instanceFolderPath + "\\" + BLOCK_VC_FILENAME_BASE + i.ToString() + "_VC_" + channelID.ToString() + ".bin";
                    using (BinaryReader br = new BinaryReader(File.Open(consolidatedDataFileName, FileMode.Open, FileAccess.Read)))
                    {
        ...
        private async void segregateDataFile(int numOfColumns, int numOfLevels, long triggerTimeStamp, int trigChannelID)
        {
            try
            {
                ...
                string folderPath = m_instanceFolderPath + "\\TraceData_";
                ...
                for (int column = 0; column < numOfColumns; column++)
                {
                    ...
                    tasks[column] = new Task<DP14MSTTaskStateObject>((stateObject) => {
                        ...
                        return tls.Value;
                    }, new DP14MSTTaskStateObject(folderPath, VCFileGenStatusEvent, percentagePerFile, column * numOfLevels, m_token), m_token);
                    ...
                    for (int level = 0; level < (numOfLevels - 1); level++)
                    {
                        continuationTasks[level][column] = parentTask.ContinueWith(antecedent => {
                            ...
                            return new DP14MSTTaskStateObject(tls.Value.FolderPath, tls.Value.FileGenerationStatusEvent, percentagePerFile, tls.Value.Index, tls.Value.Token);
                        }, m_token);
        ...
        {           
            ...
            ThreadLocal<DP14MSTTrigLocationTaskStateObject> tls = new ThreadLocal<DP14MSTTrigLocationTaskStateObject>();

            tasks[0] = new Task<int>((stateObject) => {
                tls.Value = (DP14MSTTrigLocationTaskStateObject)stateObject;
                ((DP14MSTTrigLocationTaskStateObject)tls.Value).Token.ThrowIfCancellationRequested();

                return locateTriggerStateIndex(tls.Value.TriggerTimeStamp, tls.Value.TriggerChannelID, tls.Value.FileGenerationStatusEvent);
            }, new DP14MSTTrigLocationTaskStateObject(triggerTimeStamp, trigChannelID, VCFileGenStatusEvent, 0.0f, m_token));
        ...
        private async void getSegregatedMetaData(long triggerTimeStamp, int trigChannelID)
        {
            ...
            ThreadLocal<DP14MSTGetMetaDataTaskStateObject> tls = new ThreadLocal<DP14MSTGetMetaDataTaskStateObject>();

            tasks[0] = new Task<UploadMetaData[]>((stateObject) => {
                tls.Value = (DP14MSTGetMetaDataTaskStateObject)stateObject;
                ((DP14MSTGetMetaDataTaskStateObject)tls.Value).Token.ThrowIfCancellationRequested();
            ...
            }, new DP14MSTGetMetaDataTaskStateObject(m_numOfColumns, m_numOfLevels, m_numOfChannels, m_segregatedTriggerStateIndex, trigChannelID, VCFileGenStatusEvent, 0.0f, m_token));
        ...
        {
            Task<UploadMetaData[]>[] tasks = new Task<UploadMetaData[]>[1];
            ThreadLocal<DP14MSTGetTrigStateIndicesTaskStateObject> tls = new ThreadLocal<DP14MSTGetTrigStateIndicesTaskStateObject>();

            tasks[0] = new Task<UploadMetaData[]>((stateObject) => {
                tls.Value = (DP14MSTGetTrigStateIndicesTaskStateObject)stateObject;
                ((DP14MSTGetTrigStateIndicesTaskStateObject)tls.Value).Token.ThrowIfCancellationRequested();
            ...
            return setMetaDataTriggerStateIndices(tls.Value.MetaData, tls.Value.TriggerTimeStamp, tls.Value.FileGenerationStatusEvent, tls.Value.ProgressBarPercentage); 
                ...
                //return locateTriggerStateIndex(tls.Value.TriggerTimeStamp, tls.Value.TriggerVChannelID);
            }, new DP14MSTGetTrigStateIndicesTaskStateObject(m_uploadMetaData, triggerTimeStamp, VCFileGenStatusEvent, 0.0f, m_token));
        ...
        private async void prependSegregatedMetaData(long triggerTimeStamp, int trigChannelID)
        {
            ...
            ThreadLocal<DP14MSTPrependMetaTaskStateObject> tls = new ThreadLocal<DP14MSTPrependMetaTaskStateObject>();


            for (int channel = 0; channel < VC_CHANNEL_COUNT; channel++)
            {
                tasks[channel] = new Task<bool>((stateObject) => {
                    tls.Value = (DP14MSTPrependMetaTaskStateObject)stateObject;
                    ((DP14MSTPrependMetaTaskStateObject)tls.Value).Token.ThrowIfCancellationRequested();
                    ...
                    //return locateTriggerStateIndex(tls.Value.TriggerTimeStamp, tls.Value.TriggerVChannelID);
                }, new DP14MSTPrependMetaTaskStateObject(channel, m_uploadMetaData, VCFileGenStatusEvent, 0.0f, m_token));
            }
        ...
        /// </summary>
        /// <returns></returns>
        public DP14MSTMultiThread_MetaDataArgs[] GetMultiThreadedFileMetaData()
        {
            // return (DP14MSTMultiThread_MetaDataArgs[])m_uploadMetaData.Clone();

            DP14MSTMultiThread_MetaDataArgs[] metaDataArgs = new DP14MSTMultiThread_MetaDataArgs[m_uploadMetaData.Length];
            for(int i=0; i<metaDataArgs.Length; i++)
            {
                DP14MSTMultiThread_MetaDataArgs currentArgs = new DP14MSTMultiThread_MetaDataArgs();
